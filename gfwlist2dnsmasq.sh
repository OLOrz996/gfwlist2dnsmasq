...
Valid options are:
    ...
    -il, --ikuai-list
                Convert Gfwlist into domain list for ikuai instead of dnsmasq rules
                (If this option is set, DNS IP/Port & ipset are not needed)
...
#  BASE_URL='https://raw.githubusercontent.com/Loukky/gfwlist-by-loukky/master/gfwlist.txt'
  ...
  $BASE64_DECODE $BASE64_FILE >$GFWLIST_FILE || (
    _red 'Failed to decode gfwlist.txt. Quit.\n'
    clean_and_exit 2
  )
  _green 'Done.\n\n'
  ...
  if [ ! -z $EXCLUDE_DOMAIN_FILE ]; then
    for line in $(cat $EXCLUDE_DOMAIN_FILE); do
...
ipset=/\1/'$IPSET_NAME'#g' >$CONF_TMP_FILE
  ...
  elif [ $OUT_TYPE = 'IKUAI_FILE' ]; then
    sort -u $DOMAIN_FILE >$CONF_TMP_FILE
    RULE_COUNT=1
    COUNT=1
    OUTPUT=''
    TOTAL=$(awk 'END{print NR}' $CONF_TMP_FILE)
    while read -r LINE; do
      if [ $((COUNT % 500)) = 1 ]; then
        OUTPUT=$OUTPUT'id='$RULE_COUNT' enabled=yes comment=1 domain='$LINE
      else
        OUTPUT=$OUTPUT','$LINE
        if [ $((COUNT % 500)) -eq 0 ] || [ $COUNT -eq $TOTAL ]; then
          OUTPUT=$OUTPUT' interface=wan2 src_addr= week=1234567 time=00:00-23:59\n'
          RULE_COUNT=$((RULE_COUNT + 1))
        fi
      fi
      COUNT=$((COUNT + 1))
    done <$CONF_TMP_FILE
    echo -e -n $OUTPUT >$OUT_TMP_FILE
  else
